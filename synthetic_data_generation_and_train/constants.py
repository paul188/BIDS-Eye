...
SYSTEM        — Natural-language schema description for Gemini data generation.
EXAMPLE_PAIRS — Canonical (question, sql) pairs used in Gemini prompts.

If you change the schema here, re-run the full pipeline so that generated SQL,
training data, and inference all stay aligned.
"""

# ── Schema DDL (SQLCoder format) ───────────────────────────────────────────────
# Used in the ### Database Schema block of every SQLCoder prompt.
...
# Used by collect_with_gemini.py and sample_diverse_prompts.py to instruct
# Gemini on how to generate (question, SQL) training pairs.
...
# ── Canonical example pairs ────────────────────────────────────────────────────
# Shown in Gemini prompts so it learns the expected SQL structure.
...
EXAMPLE_PAIRS = [
    ...
    {
        # Broad concept key — post-processing expands 'epilepsy_spectrum' to all sub-codes.
        # Train the LLM to use concept keys for categories, not individual sub-codes.
    ...
    {
        # Broad concept key for task — post-processing expands 'resting_state' to all sub-codes.
        ...
        "sql": (
            ...
            "    WHERE bo.dataset_id = d.id AND bo.task = 'resting_state'\n"
            ")\n"
            "GROUP BY d.id\n"
            "ORDER BY d.name"
        ),
    ...
    },
    {
        ...
        "sql": (
            ...
            "  AND EXISTS (\n"
            "    SELECT 1 FROM bids_objects bo\n"
            "    WHERE bo.dataset_id = d.id AND bo.suffix = 'bold' AND bo.task ILIKE '%rest%'\n"
            "  )\n"
            "  AND (SELECT COUNT(DISTINCT bo2.session) FROM bids_objects bo2\n"
